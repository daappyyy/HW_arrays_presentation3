...
let phoneNumbersArray = [], sumOfBalances = 0;
for(let i = 0; i < users.length; i++){
    if(users[i].balance > 2000){
        phoneNumbersArray.push(users[i].phone);
    }
}
document.write(`Phone numbers of people with balance more than $2000: <br> ${phoneNumbersArray.join(`<br>`)} <br>`);

for(let i = 0; i < users.length; i++){
    sumOfBalances+= users[i].balance;
}
document.write(`Sum of users' balances is: ${sumOfBalances.toFixed(2)} `);